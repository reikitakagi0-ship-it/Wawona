    ...
    @autoreleasepool {
        ...
        if (runtimePath) {
            ...
            if (![fm fileExistsAtPath:runtimePath isDirectory:&isDir]) {
                if (![fm createDirectoryAtPath:runtimePath withIntermediateDirectories:YES attributes:attrs error:&error]) {
                    NSLog(@"❌ Failed to create XDG_RUNTIME_DIR at %@: %@", runtimePath, error);
                    return -1;
                }
                NSLog(@"✅ Created XDG_RUNTIME_DIR at %@", runtimePath);
            } else if (!isDir) {
                NSLog(@"❌ XDG_RUNTIME_DIR exists but is not a directory: %@", runtimePath);
                return -1;
            } else {
            ...
            NSLog(@"❌ Failed to determine XDG_RUNTIME_DIR");
            return -1;
        }
