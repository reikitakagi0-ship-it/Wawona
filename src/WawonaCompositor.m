...
static int tcp_accept_handler(int fd, uint32_t mask, void *data) {
    ...
    if (client_fd >= 0) {
        ...
        BOOL allowMultiple = [[WawonaPreferencesManager sharedManager] multipleClientsEnabled];
        if (!allowMultiple && g_compositor_instance && g_compositor_instance.connectedClientCount > 0) {
        ...
        if (!client) {
            log_printf("[COMPOSITOR] ", "⚠️ Failed to create Wayland client for TCP connection\n");
            close(client_fd);
        } else {
            log_printf("[COMPOSITOR] ", "✅ Accepted TCP connection (fd=%d), created Wayland client %p\n", client_fd, client);
        }
    } else {
        ...
        if (err != EAGAIN && err != EWOULDBLOCK && err != EINTR) {
            ...
            if (err == EINVAL && !logged_inval) {
                log_printf("[COMPOSITOR] ", "⚠️ TCP accept() failed: %s (fd=%d) - socket may not be listening\n", strerror(err), fd);
                logged_inval = 1;
            } else if (err != EINVAL) {
...
static int tcp_accept_timer_handler(void *data) {
    ...
    int ret = select(compositor.tcp_listen_fd + 1, &read_fds, NULL, NULL, &timeout);
    ...
    for (int i = 0; i < max_accept_per_tick; i++) {
        ...
        struct sockaddr_in client_addr;
        socklen_t client_len = sizeof(client_addr);
        int client_fd = accept(compositor.tcp_listen_fd, (struct sockaddr *)&client_addr, &client_len);
        ...
        if (client_fd >= 0) {
            ...
            struct wl_client *client = wl_client_create(compositor.display, client_fd);
            if (!client) {
                log_printf("[COMPOSITOR] ", "⚠️ Failed to create Wayland client for TCP connection (fd=%d): %s\n", client_fd, strerror(errno));
                close(client_fd);
            } else {
                accepted_count++;
                log_printf("[COMPOSITOR] ", "✅ Accepted TCP connection (fd=%d from %s:%d), created Wayland client %p\n", 
                          client_fd, inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port), client);
            }
        } else {
            // Accept error
            int err = errno;
...
static void surface_frame(struct wl_client *client, struct wl_resource *resource, uint32_t callback) {
    ...
    // Let's assume it's a single callback for now or I should change header to support list.
    // But header is fixed.
    ...
    if (surface->buffer_resource) {
        ...
        if (shm_buffer) {
            ...
            if (egl_handler) {
                ...
                EGLint format;
                if (egl_buffer_handler_query_buffer(egl_handler, surface->buffer_resource, &width, &height, &format) == 0) {
...
static void compositor_create_surface(struct wl_client *client, struct wl_resource *resource, uint32_t id) {
    ...
    wl_resource_set_implementation(surface->resource, &surface_interface, surface, surface_destroy_resource);
...
static void compositor_create_region(struct wl_client *client, struct wl_resource *resource, uint32_t id) {
    ...
    region->resource = wl_resource_create(client, &wl_region_interface, wl_resource_get_version(resource), id);
    if (!region->resource) {
        free(region);
        wl_resource_post_no_memory(resource);
        return;
    ...
    wl_resource_set_implementation(region->resource, &region_interface, region, region_destroy_resource);
}
...
static void compositor_bind(struct wl_client *client, void *data, uint32_t version, uint32_t id) {
    ...
    BOOL allowMultiple = [[WawonaPreferencesManager sharedManager] multipleClientsEnabled];
    if (!allowMultiple && g_compositor_instance && g_compositor_instance.connectedClientCount > 0) {
        NSLog(@"🚫 Additional client connection rejected: multiple clients disabled");
    ...
    wl_resource_set_implementation(resource, &compositor_interface, compositor, compositor_destroy_bound_resource);
...
struct wl_compositor_impl *wl_compositor_create(struct wl_display *display) {
    struct wl_compositor_impl *compositor = calloc(1, sizeof(struct wl_compositor_impl));
...
void wl_compositor_set_frame_callback_requested(struct wl_compositor_impl *compositor, wl_frame_callback_requested_t callback) {
    if (compositor) compositor->frame_callback_requested = callback;
}
...
struct wl_surface_impl *wl_surface_from_resource(struct wl_resource *resource) {
    if (wl_resource_instance_of(resource, &wl_surface_interface, &surface_interface)) {
...
// Re-implementing parts of compositor_create_surface here to ensure correct linking
// (Code above was simplified)
...
- (instancetype)initWithDisplay:(struct wl_display *)display window:(NSWindow *)window {
    ...
    if (self) {
        ...
        SurfaceRenderer *renderer = [[SurfaceRenderer alloc] initWithCompositorView:compositorView];
...
int wl_send_frame_callbacks(void) {
    ...
    while (surface) {
        if (surface->frame_callback) {
            // Get current time in milliseconds (wayland time is in milliseconds since epoch)
            struct timespec ts;
...
- (BOOL)start {
    ...
    struct zwp_primary_selection_device_manager_v1_impl *primary_selection = zwp_primary_selection_device_manager_v1_create(_display);
    ...
    struct zwlr_screencopy_manager_v1_impl *screencopy = zwlr_screencopy_manager_v1_create(_display);
    if (screencopy) {
        NSLog(@"   ✓ Screencopy protocol created");
    }
    
    // Linux DMA-BUF protocol (critical for wlroots and hardware-accelerated clients)
    struct zwp_linux_dmabuf_v1_impl *linux_dmabuf = zwp_linux_dmabuf_v1_create(_display);
    if (linux_dmabuf) {
    ...
    struct zwp_idle_inhibit_manager_v1_impl *idle_inhibit = zwp_idle_inhibit_manager_v1_create(_display);
    if (idle_inhibit) {
        NSLog(@"   ✓ Idle inhibit protocol created");
    }
    
    // Pointer gestures protocol (trackpad gestures)
    struct zwp_pointer_gestures_v1_impl *pointer_gestures = zwp_pointer_gestures_v1_create(_display);
    if (pointer_gestures) {
        NSLog(@"   ✓ Pointer gestures protocol created");
    }
    
    // Relative pointer protocol (relative motion for games)
    struct zwp_relative_pointer_manager_v1_impl *relative_pointer = zwp_relative_pointer_manager_v1_create(_display);
    if (relative_pointer) {
        NSLog(@"   ✓ Relative pointer protocol created");
    }
    
    // Pointer constraints protocol (pointer locking/confining for games)
    struct zwp_pointer_constraints_v1_impl *pointer_constraints = zwp_pointer_constraints_v1_create(_display);
    if (pointer_constraints) {
        NSLog(@"   ✓ Pointer constraints protocol created");
    }
    
    // Register additional protocols
    struct zwp_tablet_manager_v2_impl *tablet = zwp_tablet_manager_v2_create(_display);
    (void)tablet; // Suppress unused variable warning
    NSLog(@"   ✓ Tablet protocol created");
    
    struct ext_idle_notifier_v1_impl *idle_manager = ext_idle_notifier_v1_create(_display);
    ...
    struct org_kde_plasma_shell_impl *plasma_shell = org_kde_plasma_shell_create(_display);
    ...
    struct qt_surface_extension_impl *qt_surface = qt_surface_extension_create(_display);
    if (qt_surface) {
    ...
    _eventThread = [[NSThread alloc] initWithBlock:^{
        ...
        @try {
            ...
            if (compositor.tcp_listen_fd >= 0) {
                ...
                tcp_accept_timer = wl_event_loop_add_timer(eventLoop, tcp_accept_timer_handler,
                                                          (__bridge void *)compositor);
                
                if (tcp_accept_timer) {
                    // Start timer immediately (0ms) - it will reschedule itself via return value
                    int ret = wl_event_source_timer_update(tcp_accept_timer, 0);
                    if (ret < 0) {
                        log_printf("[COMPOSITOR] ", "⚠️ Failed to start TCP accept() timer: %s\n", strerror(errno));
                        wl_event_source_remove(tcp_accept_timer);
                        tcp_accept_timer = NULL;
                    } else {
                        log_printf("[COMPOSITOR] ", "✅ TCP accept() timer registered and started (listen_fd=%d, ret=%d)\n", compositor.tcp_listen_fd, ret);
                    }
                } else {
                    log_printf("[COMPOSITOR] ", "⚠️ Failed to register TCP accept() timer\n");
                }
            }
            ...
            while (!compositor.shouldStopEventThread) {
                ...
                if (compositor.tcp_listen_fd >= 0) {
                    ...
                    int select_ret = select(compositor.tcp_listen_fd + 1, &read_fds, NULL, NULL, &timeout);
